...
macro_rules! config {
    ($config:ident, $result:ident, $config_type:ident) => {
        let &configuration::$config_type($result) =
...
#[derive(Debug)]
pub struct SceneWalls(Vec<SceneWall>);
#[derive(Debug)]
...
pub fn new(config_filename: &str) -> AnyMap {
    let mut config = AnyMap::new();
    parse_config_file(&mut config, config_filename);
    config
}

fn parse_config_file(config : &mut AnyMap, config_filename: &str) {
    info!("Starting to parse config file {}", config_filename);
    ...
    let mut done = false;
    while !done {
        done = !parse_single_item(config, &mut file, &mut buf);
    }
    info!("Config readed.");
}

fn parse_single_item(config : &mut AnyMap, file : &mut File, buf : &mut [u8]) -> bool {
...
fn parse_scene_item(config : &mut AnyMap, file : &mut File, buf : &mut [u8]) {
...
fn parse_time_item(config : &mut AnyMap, file : &mut File, buf : &mut [u8]) {
    let element = parse_u16(file, buf);
    match element {
        0x01 => {
...
fn parse_spawn_item(config : &mut AnyMap, file : &mut File, buf : &mut [u8]) {
    let element = parse_u16(file, buf);
    match element {
        0x01 => {
...
fn parse_forces_item(config : &mut AnyMap, file : &mut File, buf : &mut [u8]) {
...
fn parse_repulsion_force_item(config : &mut AnyMap, file : &mut File, buf : &mut [u8]) {
...
fn parse_target_force_item(config : &mut AnyMap, file : &mut File, buf : &mut [u8]) {
    let element = parse_u8(file, buf);
    match element {
        0x01 => {
            let distribution = parse_distribution(file, buf);
...
fn parse_coordinates(file : &mut File, buf : &mut [u8]) -> (u16, u16, u16, u16) {
...
fn parse_distribution(file : &mut File, buf : &mut [u8]) -> DistributionValue {
